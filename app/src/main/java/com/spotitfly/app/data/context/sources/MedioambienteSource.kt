...
/**
 * Medioambiente por punto (ZG/FeatureServer/1) — paridad iOS:
 *  - Título fijo: "Zona Medioambiental"
 ...
 */
internal object MedioambienteSource {
    private const val TAG = "MedioambienteSource"

    private const val ENDPOINT_ZG_ENV =
        "https://servais.enaire.es/insignia/rest/services/NSF_SRV/SRV_UAS_ZG_V1/FeatureServer/1/query"
    ...
    fun fetch(lat: Double, lng: Double): List<Pair<String, String?>> {
        ...
        runCatching {
            ...
            Log.d(TAG, "MEDIO(geojson) → ${buildUrl(ENDPOINT_ZG_ENV, params)}")
            val t0 = System.currentTimeMillis()
            val root = Http.getJson(ENDPOINT_ZG_ENV, params)
            val dt = System.currentTimeMillis() - t0
            Log.d(TAG, "MEDIO(geojson) ← ${dt}ms; rootKeys=${keys(root)}")

            val feats = root.optJSONArray("features") ?: JSONArray()
            Log.d(TAG, "MEDIO features=${feats.length()} (geojson→properties)")
            for (i in 0 until feats.length()) {
                val p = feats.getJSONObject(i).optJSONObject("properties") ?: JSONObject()
                if (i == 0) Log.d(TAG, "MEDIO first.properties.keys=${keys(p)}")

                // iOS: título fijo
                val title = "Zona Medioambiental"
        ...
        }.onFailure { Log.w(TAG, "MEDIO error: ${it.message}", it) }
